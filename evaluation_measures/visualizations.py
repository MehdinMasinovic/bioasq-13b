...
def plot_metrics(metrics, traditional, neural_nlp, title):
    ...
    filename = "trad_vs_neu_flat" if 'Flat' in title else "trad_vs_neu_hier"
    plt.savefig(f'{filename}.png')  # Save the figure
    plt.savefig(f'{filename}.pdf')  # Save the figure
...
def plot3(flat_labels, hier_labels, flats, hiers, focus_labels, legend):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('comparison_f_scores.png')  # Save the figure
    plt.savefig('comparison_f_scores.pdf')  # Save the figure
...
# Plot flat metrics
plot_metrics(flat_metrics, flat_tad, flat_neural, 'Flat Metrics Comparison')
# Plot hierarchical metrics
plot_metrics(hier_metrics, hier_tad, hier_neural, 'Hierarchical Metrics Comparison')
...
plot3(flat_metrics, hier_metrics, [flat_tad,flat_neural,flat_tradcutoff,flat_hybrid], [hier_tad,hier_neural,hier_tradcutoff,hier_hybrid], 
        ['EbF','MaF','MiF','hF','LCA-F'], ['Traditional IR', 'Neural NLP', 'Trad + Cutoff', 'Trad + Rerank'])
